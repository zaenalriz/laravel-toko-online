    ...
    <div class="card shadow mb-4">
        <div class="card-body">
            ...
            @if ($errors)
                ...
                @endforeach
            @endif
            <form class="user" action="{{ route('backend.blog.aksi_edit',$artikel->id) }}" enctype="multipart/form-data" method="post">
                @csrf
                <div class="form-group">
                    <input type="text" name="title" class="form-control form-control-user" aria-describedby="emailHelp"
                        placeholder="Masukan judul" value="{{$artikel->title}}">
                </div>
                <div class="form-group">
                    <textarea name="description" class="form-control editor" placeholder="masukan deskripsi" cols="30" rows="3">{{$artikel->description}}</textarea>
                </div>
                <div class="form-group">
